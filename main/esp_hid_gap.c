...
/* 由 ble_hid.c 实现：记录已配对/已连接主机地址，供蓝牙重置时主动断开 */
extern void ble_hid_record_peer(esp_bd_addr_t bda);
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        if (!param->ble_security.auth_cmpl.success) {
            ...
            /* 防御：配对/加密失败时，本端已无 LTK，但 Windows 可能已建立连接并
             * 持续触发重连风暴(ltk_req_neg_reply → 断开 → 再重连)。主动断开对端，
             * 让主机明白本次配对失败，需要重新配对，从而打破死循环。 */
            esp_ble_gap_disconnect(param->ble_security.auth_cmpl.bd_addr);
            ESP_LOGW(TAG, "已主动断开加密失败的对端，避免重连风暴");
        } else {
            ESP_LOGI(TAG, "BLE GAP AUTH SUCCESS");
            /* 记录对端地址，蓝牙重置时用于主动断开连接 */
            ble_hid_record_peer(param->ble_security.auth_cmpl.bd_addr);
        }
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    /* 本项目改动：设备上没有屏幕也没有键盘输入，采用 Just Works 免密码配对，
     * 这样在 Windows 上点击"添加设备"后无需输入或比对数字即可直接配对成功。 */
...
/**
 * @brief 为"重置蓝牙"场景轮换一个全新的随机蓝牙地址
 *
 * 为什么要换地址：商业无线键鼠在清除配对时，会让设备 MAC 地址轮换。
 * 否则对端(Windows)仍保存着旧配对，会不断自动重连；重连后本端 LTK 已删除
 * 导致加密失败(ltk_req_neg_reply)→ 断开 → 再重连，形成重连风暴。
 * 换成新地址后，Windows 旧配对对应的是老地址，自然匹配不上，必须重新配对。
 *
 * 注意：静态随机地址的规范要求最高 2 bit 必须为 '11'，否则控制器会拒绝设置。
 */
esp_err_t esp_hid_ble_use_new_random_addr(void)
{
    esp_bd_addr_t rand_addr;
    /* 取随机数填充，再把最高 2 位强制设为 11（静态随机地址标志） */
    esp_fill_random(rand_addr, sizeof(rand_addr));
    rand_addr[5] |= 0xC0;   /* set the two most significant bits of the last byte to 1 */
    rand_addr[5] &= 0xDF;   /* clear bit 6 (0b11xxxx, not 0b111xxxxx) */

    esp_err_t ret = esp_ble_gap_set_rand_addr(rand_addr);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "已轮换随机地址 %02x:%02x:%02x:%02x:%02x:%02x",
                 rand_addr[0], rand_addr[1], rand_addr[2],
                 rand_addr[3], rand_addr[4], rand_addr[5]);
    } else {
        ESP_LOGE(TAG, "设置随机地址失败: %s", esp_err_to_name(ret));
    }
    return ret;
}
#endif /* CONFIG_BT_BLE_ENABLED */
