...
| KEY1+KEY3 | bit2(P1_6)+bit0(P1_4) | 同时长按 3 秒 → 先 `action_engine_stop_and_wait()`，再 `ble_hid_reset()`（组合键生效期间屏蔽 KEY3 单按复位，避免误触发） |
...
```
1. init_nvs             NVS（含 BLE 绑定 + 新增 km_cfg 命名空间）
2. led_status_init
3. hid_sender_start
4. ble_hid_init          BLE 广播
5. action_engine_start_task
6. keys_start            物理按键控制面（KEY0/KEY2/KEY3/KEY1+KEY3 语义不变）
7. config_store_load    从 NVS 读取 km_cfg，无则写默认
8. wifi_manager_start   常开 SoftAP + 按配置并发连 STA
...
结构含：`version / run_mode / weights(五类权重) / sequence(≤32 项+循环策略) / timers(≤8 条) / wifi(STA 凭据)`。

### 8.3 Web 控制台功能

- **状态栏**：AP IP、STA IP、蓝牙连接状态、引擎运行状态、启动/停止按钮（每 3 秒轮询 `/api/status`）。
- **运行模式**：随机模式（五类权重数字输入，实时显示合计）/ 序列模式（切换选项卡）。
...
flowchart TD
    ...
    I -- 未打断 --> B
    G -- 是 --> J[LED 亮]
    J --> K[pick_action 加权随机抽动作<br/>权重 10/15/35/20/20]
    K --> L{抽到哪个动作?<br/>权重 15/20/40/20/5}
    L -- 拖拽15% --> M1[act_drag]
    L -- 点击20% --> M2[act_click]
    L -- 滚轮40% --> M3[act_wheel]
    L -- 方向键20% --> M4[act_arrow_keys]
    L -- 休息5% --> M5[act_rest]
...
```mermaid
flowchart TD
    A[重复次数 R = 5~10] --> B[按下鼠标左键]
    B --> C{i < R?}
    C -- 是 --> D[随机目标点(范围内)<br/>mouse_move_to 分片步进<br/>每段间隔 ACTION_MOVE_STEP_MS=8ms]
...
```mermaid
flowchart TD
    A[随机目标点(范围内) 步进移动一下] --> B[重复次数 R = 8~25]
    B --> C{i < R?}
...
```mermaid
flowchart TD
    A[随机延迟 1000~30000ms<br/>action_delay_ms] --> B[exit: action_release_all]
```
...
- 移动采用 `mouse_move_to()`：从当前逻辑位置**分片步进**到目标点（每帧 ≤120 像素，帧间可中断延迟），绝不瞬移，因此光标不会"闪烁"跳过，也不会飘出活动范围；
...
| 宏名 | 含义 | 当前值 |
|---|---|---|
| `ACT_W_DRAG` | 拖拽概率权重 | 15 |
| `ACT_W_CLICK` | 点击概率权重 | 20 |
| `ACT_W_WHEEL` | 滚轮概率权重 | 40 |
| `ACT_W_ARROW` | 方向键概率权重 | 20 |
| `ACT_W_REST` | 休息概率权重 | 5 |
| `DRAG_DIST_MIN/MAX` | 拖拽距离（预留，当前未直接使用） | 5 / 100 |
| `DRAG_STEP_MIN/MAX` | 拖拽步长（预留，当前未直接使用） | 5 / 10 |
| `DRAG_REPEAT_MIN/MAX` | 拖拽重复次数 | 5 / 10 |
| `DRAG_INTERVAL_MIN/MAX` | 拖拽间隔(ms) | 100 / 500 |
| `DRAG_END_DELAY_MIN/MAX` | 拖拽结束延迟(ms) | 100 / 500 |
| `CLICK_STEP_MIN/MAX` | 点击步长（预留，当前未直接使用） | 10 / 50 |
| `CLICK_REPEAT_MIN/MAX` | 点击重复次数 | 5 / 10 |
| `CLICK_INTERVAL_MIN/MAX` | 点击间隔(ms) | 300 / 1000 |
| `CLICK_END_DELAY_MIN/MAX` | 点击结束延迟(ms) | 300 / 1000 |
| `WHEEL_STEP_MIN/MAX` | 滚轮移动步长（预留，当前未直接使用） | 5 / 30 |
| `WHEEL_REPEAT_MIN/MAX` | 滚轮重复次数 | 8 / 25 |
| `WHEEL_INTERVAL_MIN/MAX` | 滚轮间隔(ms) | 50 / 500 |
| `WHEEL_TICK_MIN/MAX` | 滚轮步进 | 1 / 5 |
| `WHEEL_END_DELAY_MIN/MAX` | 滚轮结束延迟(ms) | 50 / 500 |
| `ARROW_REPEAT_MIN/MAX` | 方向键重复次数 | 10 / 35 |
| `ARROW_INTERVAL_MIN/MAX` | 方向键间隔(ms) | 50 / 1000 |
| `ARROW_END_DELAY_MIN/MAX` | 方向键结束延迟(ms) | 10 / 100 |
| `REST_DELAY_MIN/MAX` | 休息延迟(ms) | 1000 / 30000 |
| `MOUSE_POS_LIMIT_X` | 鼠标水平活动半范围基准(像素) | 500 |
| `MOUSE_POS_LIMIT_Y` | 鼠标垂直活动半范围基准(像素) | 200 |
| `SCREEN_SCALE_PCT` | 屏幕显示缩放百分比 | 125 |
| `MOUSE_POS_EFF_X` | 折算缩放后的有效水平半范围（派生） | `500*100/SCALE` |
| `MOUSE_POS_EFF_Y` | 折算缩放后的有效垂直半范围（派生） | `200*100/SCALE` |
| `MOUSE_HOME_CORNER` | 复位撞角 `CORNER_TOP_RIGHT/LEFT/BOTTOM_RIGHT/LEFT` | `CORNER_TOP_RIGHT` |
| `MOUSE_HOME_PUSH_PX` | 撞角朝角落方向发的总位移计数（确保越界被截停） | 1500 |
| `MOUSE_HOME_BACK_X` | 回中心水平位移计数（决定水平落点，待实测） | 600 |
| `MOUSE_HOME_BACK_Y` | 回中心垂直位移计数（1080@125% 实测 200 居中） | 200 |
| `ACTION_GUARD_DELAY_MS` | 动作间保护延迟(ms) | 100 |
| `ACTION_MOVE_STEP_MS` | 分片位移帧间延迟(ms) | 8 |
| `ACTION_HOME_STEP_MS` | 复位撞角帧间延迟(ms) | 5 |
...
> 概率权重合计应为 100，但代码用 `esp_random() % total` 取模，所以即使合计不是 100 也能正常工作，只是相对比例变化。
