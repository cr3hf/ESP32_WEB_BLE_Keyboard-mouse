...
- **运行模式**：随机模式（九类权重数字输入，实时显示合计）/ 序列模式（切换选项卡）。
- **动作序列编排**：从九类动作添加/上移/下移/删除；循环策略可选「顺序循环」或「乱序洗牌」。
- **文本输入（写文本）**：位于「⌨️ 打字单词表」卡片内、单词文本框下方。可粘贴/编辑文本、上传 TXT 替换、导出 TXT 下载、复制全文、清空；点「保存文本」单独写入（与主配置分开保存），超出 128K 字符时禁止保存。
...
|---|---|---|
| `/` | GET | 内置控制台页面 |
| `/api/brand` | GET | 品牌信息 `{app_title, export_prefix, ble_name}`（公开，无需登录；页面标题与导出文件名前缀由此下发） |
| `/api/status` | GET | `{ap_ip, sta_ip, bt, running, action, progress, rest_remaining, time, text_pos, text_len}`（`text_pos`/`text_len` = 写文本输出位置/总长，供页脚状态栏显示） |
| `/api/config` | GET | 当前配置 JSON |
| `/api/config` | POST | 保存配置并实时应用到引擎 |
| `/api/text` | GET | 返回“写文本”动作使用的原始文本（text/plain，供页面加载/导出 TXT） |
| `/api/text` | POST | 原始请求体即文本（限 128K 字节），写入 `textdb` 分区并复位续写游标 |
| `/api/text/cursor` | POST | `{"pos":N}` 设置「写文本」输出起始位置（`action_engine_set_text_cursor`，仅内存不持久化，返回当前 `text_pos`/`text_len`） |
...
### 8.7 长文本存储（`main/text_store.c`）

- “写文本”动作使用的一段文本最长 128K 字符，远超 NVS（仅 24KB），故**不放进 `config_t`/NVS**，而是单独存放在 Raw Flash 分区 `textdb`（data 类、子类型 0x40、大小 0x40000=256KB，见 `main/partitions_ota.csv`）。
- 分区内布局：`[0..3] magic "TXTD"` + `[4..7] length` + `[8..] 文本字节`（4 字节对齐补零）。
- 启动时 `text_store_init()` 整段读入 **PSRAM** 缓冲（失败回退内部 RAM），供动作引擎逐字符输出；若分区为空则写入内置默认文本（工程根 `Note.txt`，构建期由 `main/gen_default_text.py` 生成 `main/default_text.h`）。
- 续写游标 `s_text_cursor` 仅存于内存（不持久化）：达到每轮随机上限即保存游标、下次续写；写到末尾或文本被修改保存后归零。
- 游标也可由页面手动设置：文本框下方「起始文档位置」+「设为起点」→ `POST /api/text/cursor` → `action_engine_set_text_cursor()`（默认 0=从头，越界按文本长度截断，不影响本轮已在执行的输出）。
- 注意：改动分区表后需 `idf.py fullclean` 再重编烧录。
...
flowchart TD
    ...
    I -- 未打断 --> B
    G -- 是 --> J[LED 亮]
    J --> K[pick_action 加权随机抽动作<br/>9 类权重 drag/click/wheel/arrow/rest/move/word/alt_tab/text<br/>= 0/50/50/30/35/50/0/0/0（合计不必为100，仅相对比例；0=不参与抽取）]
    K --> L{抽到哪个动作?}
    L -- 拖拽0 --> M1[act_drag]
    L -- 点击50 --> M2[act_click]
    L -- 滚轮50 --> M3[act_wheel]
...
```mermaid
flowchart TD
    A[随机延迟 rest_delay_min~max ×100ms<br/>（默认 10~200 = 1~20s）<br/>action_delay_ms] --> B[累计连续休息时长 s_consec_rest_ms]
    B --> C[exit: action_release_all]
```

> 休息也是一个正常动作：被抽中后，引擎在 `act_rest()` 内部等待，期间 LED 保持亮（仍在 RUN 态），延迟可被 STOP/断连中断。
> **连续休息上限**：累计连续休息超过 `rest_max_total_sec`（默认 540 秒=9 分钟，0=不限）后：
> ① 主循环若再抽到休息，会由 `apply_rest_cap()` 改为随机执行一个非休息动作；
> ② **正在进行的休息也会立即被打断**——`act_rest()` 内部按 `REST_SLICE_MS`(100ms) 分段等待，每段用实测时长累加 `s_consec_rest_ms` 并检查上限，达到即跳出（日志：`[连续休息限制] …立即打断本次休息`）。
> 该累计计时**仅在执行非休息动作时清零**（`exec_action_by_id()` 中处理）。
...
> **复位触发规则（当前版本）**：四个“移动鼠标”动作（拖拽 `ACT_DRAG` / 点击 `ACT_CLICK` / 滚轮 `ACT_WHEEL` / 滑动 `ACT_MOVE`）**各自**由一个开关决定是否在**本动作开始前**先复位一次：`drag_home_before` / `click_home_before` / `wheel_home_before` / `move_home_before`（默认 **1=勾选**，可在 Web「动作参数」对应动作组中修改）。
> 该判断由 `action_needs_home()` 统一完成；**复位只在动作开始前做一次，动作内部的循环 / 分片不复位**。其余动作（方向键 / 休息 / 打字 / 切换程序 / 写文本）不涉及复位。
> 该规则在三个执行入口统一实现：随机/序列主循环（`action_engine_task`）、序列单轮触发（`action_engine_trigger_sequence_once`）、定时单动作（`action_engine_run_single`）。
> `KEY3` 短按的 `action_engine_stop_and_reset()` 仍会手动复位一次（独立于上述规则）。
...
> **重要：宏只是“编译期默认值”**。这些宏在固件编译时被固化，决定设备**首次上电 / 恢复出厂**时的初始参数；但**它们不是唯一改参数的途径**。设备运行后，所有参数都可在 **Web 配置控制台（详见 §8 / Web 控制台小节）** 上直接调：运行模式、八类权重、动作序列与循环策略、定时任务、动作时间、运动模式、WiFi 配网、3 套参数套等，改完点「保存配置」即写回 NVS 并实时生效，**无需重新编译烧录**。
> 换句话说：**改默认值 → 改宏 + 重编译烧录；运行时调参 → 上网页**。网页保存的值会覆盖宏默认，并持久化在 NVS，断电不丢。
...
| `REST_DELAY_MIN/MAX` | 休息延迟（单位 ×100ms，60=6秒） | 10 / 200 |
| `rest_max_total_sec`（可配，页可改） | 连续休息累计上限（秒，0=不限）；超限后强制改执行非休息动作，且休息中立即被打断；仅非休息动作清零计时 | 540 |
| `REST_SLICE_MS` | 休息分段等待粒度(ms)，用于休息期间累计检查与立即打断 | 100 |
...
| `drag/click/wheel/move_home_before`（可配，页可改） | 四个“移动鼠标”动作“执行前是否先做一次鼠标复位”（1=是 0=否），**动作内部循环不复位** | 1 / 1 / 1 / 1 |
...
| `TEXT_CHAR_DELAY_MIN/MAX` | 写文本字符间延迟(ms) | 40 / 300 |
| `TEXT_LINE_DELAY_MIN/MAX` | 写文本换行处延迟(ms) | 80 / 600 |
| `TEXT_END_DELAY_MIN/MAX` | 写文本结束延迟(ms) | 500 / 2000 |
| `TEXT_CHAR_LIMIT_MIN/MAX` | 写文本每轮输出字符数上下限(个) | 1000 / 5000 |
| `text_pre_pagedown_count`（可配，页可改） | 写文本每轮开头前置 PageDown 次数（仅 PageDown，不发 End） | 16 |
| `text_skip_line_indent`（可配，页可改） | 写文本：1=回车后屏蔽行首空格/制表符（防编辑器自动缩进重复缩进） | 0 |
| `TEXT_PRE_KEY_HOLD_MS / GAP_MS` | 前置 PageDown/End 按下保持 / 间隔(ms) | 10 / 20 |
...
- **改完宏后必须重新编译（建议 fullclean 后 build）并烧录**，宏值是在编译期固化进固件的——它决定的是**默认值**（首次上电 / 恢复出厂时的初始参数）。
- **运行时真正调参请上 Web 页面**：宏只是默认值来源，设备跑起来后，运行模式、八类权重、序列、定时、动作时间、运动模式、WiFi、参数套等都可在 `192.168.4.1` 的 Web 控制台直接改，点「保存配置」即写回 NVS 并实时生效，无需重编译。网页保存的值会覆盖宏默认（断电不丢），只有想要“改一套全新的出厂默认”时才需要动宏并重编译烧录。
